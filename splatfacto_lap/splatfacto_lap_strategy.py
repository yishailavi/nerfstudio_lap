...
@dataclass
class SplatfactoLapStrategy(Strategy):
    ...
    def _update_state(
        ...
        if state["grad2d"].shape[0] != n_gaussian:
            tmp = state["grad2d"].clone()
            state["grad2d"] = torch.zeros(n_gaussian, device=grads.device)
            state["grad2d"][:tmp.shape[0]] = tmp
        if state["count"] is None:
            state["count"] = torch.zeros(n_gaussian, device=grads.device)
        if state["count"].shape[0] != n_gaussian:
            tmp = state["count"].clone()
            state["count"] = torch.zeros(n_gaussian, device=grads.device)
            state["count"][:tmp.shape[0]] = tmp
        if self.refine_scale2d_stop_iter > 0 and state["radii"] is None:
            assert "radii" in info, "radii is required but missing."
            state["radii"] = torch.zeros(n_gaussian, device=grads.device)
        if self.refine_scale2d_stop_iter > 0 and state["radii"].shape[0] != n_gaussian:
            tmp = state["radii"].clone()
            state["radii"] = torch.zeros(n_gaussian, device=grads.device)
            state["radii"][:tmp.shape[0]] = tmp
