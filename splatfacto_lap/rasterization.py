...
def rasterization(
    ...
    levels: Optional[Tensor] = None,
    bin_mask: Optional[Tensor] = None,
    ...
    if bin_mask is not None:
        rounded_coords = means2d.round().long()
        y_coords = rounded_coords[0, :, 0]
        x_coords = rounded_coords[0, :, 1]
        H, W = bin_mask.shape
        valid = (x_coords >= 0) & (x_coords < H) & (y_coords >= 0) & (y_coords < W)
        arr = torch.zeros(rounded_coords.shape[1], dtype=torch.bool, device=device)
        arr[valid] = bin_mask[x_coords[valid], y_coords[valid]]
        arr |= levels == 0
        radii[0, ~arr] = 0

    if fov_mask is not None:
        if compensations is None:
            compensations = torch.ones_like(opacities, device=device)
        unique_levels = torch.unique(levels)
        _N, _H, _W = fov_mask.shape
        rounded_coords = means2d.round().long()
        y_coords = rounded_coords[0, :, 0]
        x_coords = rounded_coords[0, :, 1]
        valid = (x_coords >= 0) & (x_coords < _H) & (y_coords >= 0) & (y_coords < _W)
        for level in unique_levels:
            idx = levels == level
            compensations[0, valid & idx] = compensations[0, valid & idx] * fov_mask[int(level)][x_coords[valid & idx], y_coords[valid & idx]]
        compensations[0, (radii[0] > 0) & (~valid) & (levels > 0)] = 0.

    if compensations is not None:
        opacities = opacities * compensations
        # opacities = 1 - opacities * compensations
        # radii[0, opacities[0] <= 0.001] = 0
    ...
    if sh_degree is None:
        ...
        colors[:, levels > 0, :] = colors[:, levels > 0, :] * 2 - 1
